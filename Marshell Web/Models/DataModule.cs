...
namespace DataBase
{
    public static class DataModule
    {
        ...
        private static readonly int ProductCritical = 25; // should store in web config 

        // Correctly typed MySqlControl instance (replace the incorrect string declaration)
        private static readonly MySqlControl MySqlCtl = new MySqlControl();
        ...
        public static List<Customers> GetCustomers(string keyword = "")
        {
            ...
            try
            {
                ...
                using (MySqlConnection cc = new MySqlConnection(        ))
                {
                    cc.Open();
        ...
        public static List<Customers> GetOwingCustomers(string keyword = "")
        {
            List<Customers> cs = new List<Customers>();
            //[Obsolute]
            //MySqlControl msql = new MySqlControl(GetConnectionString());
            try
            {
                string param = "";
                if (keyword == "") //if nothing then get all data
        ...
        public static void SaveCustomer(Customers cus)
        {
            try
            {
                ...
                using (MySqlConnection cc = new MySqlConnection(GetConnectionString()))
                {
                    cc.Open();
                    using (MySqlCommand cm = new MySqlCommand(qr, cc))
                    {
    ...
    public static List<Users> GetEmployees(string keyword = "")
    {
        ...
        MySqlControl db = new MySqlControl(GetConnectionString());
        try
        {
            string param = "";
            if (keyword == "")
    ...
    public static List<Products> GetProductsLow()
    {
        int low = 10;//init
        //replace default 10
        low = System.Convert.ToInt32(ProductCritical); 
        List<Products> lp = new List<Products>();
        try
        {
        ...
        {
           Console.WriteLine(ex.Message + Environment.NewLine + " Fail to Load Understock Product Products");
        }
#else
        catch (Exception)
        { }
    ...
    public static List<Invoices> GetInvoicesDateToDate(DateTime D1, DateTime D2)
    {
        ...
        {
           Console.WriteLine(ex.Message + Environment.NewLine + "Fail to Load INVOICES BY DATES");
        }
#else
        catch (Exception)
        { }
    ...
    public static List<Invoices> GetInvoicesThisweek()
    {
        ...
        {
           Console.WriteLine(ex.Message + Environment.NewLine + "Fail to Load INVOICES FOR THIS WEEK");
        }
#else
        catch (Exception)
        { }
    ...
    public static List<Invoices> GetInvoicesThisMonth()
    {
        ...
        {
           Console.WriteLine(ex.Message + Environment.NewLine + "Fail to Load INVOICES FOR THIS MONTH");
        }
#else
        catch (Exception)
        { }
        ...
        try
        {
            using (MySqlConnection cn = new MySqlConnection(GetConnectionString()))
            {
                ...
                using (MySqlCommand cm = new MySqlCommand("INSERT INTO CREDITS(BARCODE,ITEMID,CDATE,QNT,OWIN,STAT) VALUES(@bc,@pid,@cdat,@qn,@ow,@stat)", cn))
                {
                    ...
                    if (i == 0)
                    {
                       Console.WriteLine("Fail To add product Detials to Facturas");
                    }
                    else
    ...
    /// <summary>
    /// Reads and returns the connection string named "Mashelldbcon" from web.config / app.config.
    /// Returns empty string if not found or on error.
    /// </summary>
    public static string GetMashelldbConnectionString()
    {
        try
        {
            var cs = ConfigurationManager.ConnectionStrings["Mashelldbcon"]?.ConnectionString;
            if (string.IsNullOrWhiteSpace(cs))
            {
                Console.WriteLine("Connection string 'Mashelldbcon' not found in configuration.");
                return string.Empty;
            }
        ...
        {
            Console.WriteLine("Error reading 'Mashelldbcon' from configuration: " + ex.Message);
            return string.Empty;
        }
    }
